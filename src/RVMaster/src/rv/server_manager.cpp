...
extern void initAdvertiseOptions(std::string topic,
                                 ros::AdvertiseOptions &ops_pub);
// extern void initSubscribeOptions(std::string topic, ros::SubscribeOptions
// ops_sub);
}
...
ServerManager::~ServerManager() { shutdown(); }
...
ros::PublicationPtr ServerManager::getOrCreatePubPtr(std::string topic) {
  if (pubptrMap.find(topic) == pubptrMap.end()) // no pub_ptr available
  {
    ...
    ros::PublicationPtr pub_ptr_ = ros::PublicationPtr(
        new ros::Publication(ops_pub.topic, ops_pub.datatype, ops_pub.md5sum,
                             ops_pub.message_definition, ops_pub.queue_size,
                             ops_pub.latch, ops_pub.has_header));
    ros::SubscriberCallbacksPtr callbacks(new ros::SubscriberCallbacks(
        ops_pub.connect_cb, ops_pub.disconnect_cb, ops_pub.tracked_object,
        ops_pub.callback_queue));
...
void ServerManager::publish(std::string topic,
                            ros::SerializedMessage serializedMsg) {
...
void ServerManager::requestTopicCallback(XmlRpc::XmlRpcValue &params,
                                         ClientInfo &ci,
...
bool ServerManager::requestTopic(const string &topic,
                                 XmlRpc::XmlRpcValue &protos,
                                 XmlRpc::XmlRpcValue &ret) {
  ROS_INFO("Requesting topic: %s", topic.c_str());
}
void ServerManager::pubUpdateCallback(XmlRpc::XmlRpcValue &params,
                                      ClientInfo &ci,
                                      XmlRpc::XmlRpcValue &result) {
...
bool ServerManager::getPublishedTopicsCallback(XmlRpc::XmlRpcValue &params,
                                               ClientInfo &ci,
                                               XmlRpc::XmlRpcValue &result) {
  // 0: node_name
  ...
  if (acctrl::isCommandAllowed(command, node_name, ci.ip)) {
    ...
    */
    ...
    ROS_INFO("Node %s successfully getPublishedTopics from %s",
             node_name.c_str(), ci.ip.c_str());
    return true;
  } else {
    ROS_WARN("Node %s is not able to getPublishedTopics from %s due to access "
             "control!",
             node_name.c_str(), ci.ip.c_str());
...
bool ServerManager::getUriCallback(XmlRpc::XmlRpcValue &params, ClientInfo &ci,
                                   XmlRpc::XmlRpcValue &result) {
  string node_name = params[0];

  // keep a map from node_name to url

  ROS_INFO("Node %s trying to getUri from %s", node_name.c_str(),
           ci.ip.c_str());
  ...
  if (acctrl::isCommandAllowed(command, node_name, ci.ip)) {
    ...
    ROS_INFO("Node %s successfully getUri from %s", node_name.c_str(),
             ci.ip.c_str());
    return true;
  } else {
    ROS_WARN("Node %s is not able to getUri from %s due to access control!",
             node_name.c_str(), ci.ip.c_str());
...
bool ServerManager::getPidCallback(XmlRpc::XmlRpcValue &params, ClientInfo &ci,
                                   XmlRpc::XmlRpcValue &result) {
  // 0: node_name
  ...
  ROS_INFO("Node %s trying to getPid from %s", node_name.c_str(),
           ci.ip.c_str());
  ...
  if (acctrl::isCommandAllowed(command, node_name, ci.ip)) {
    ...
    ROS_INFO("Node %s successfully getPid from %s", node_name.c_str(),
             ci.ip.c_str());
    return true;
  } else {
    ROS_WARN("Node %s is not able to getPid from %s due to access control!",
             node_name.c_str(), ci.ip.c_str());
...
bool ServerManager::getTopicTypesCallback(XmlRpc::XmlRpcValue &params,
                                          ClientInfo &ci,
                                          XmlRpc::XmlRpcValue &result) {
  // 0: node_name
  ...
  ROS_INFO("Node %s trying to getTopicTypes from %s", node_name.c_str(),
           ci.ip.c_str());
  ...
  if (acctrl::isCommandAllowed(command, node_name, ci.ip)) {
    ...
    ROS_INFO("Node %s successfully getTopicTypes from %s", node_name.c_str(),
             ci.ip.c_str());
    return true;
  } else {
    ROS_WARN(
        "Node %s is not able to getTopicTypes from %s due to access control!",
        node_name.c_str(), ci.ip.c_str());
...
bool ServerManager::getMonitorsCallback(XmlRpc::XmlRpcValue &params,
                                        ClientInfo &ci,
                                        XmlRpc::XmlRpcValue &result) {
  ...
  if (acctrl::isCommandAllowed(command, node_name, ci.ip)) {
    ...
    int i = 0;
    for (iter = monitor::allMonitors.begin();
         iter != monitor::allMonitors.end(); iter++) {
      string monitor_name = *iter;

      XmlRpc::XmlRpcValue monitor_value;
      monitor_value[0] = monitor_name;
      if (monitor::enabledMonitors.find(monitor_name) !=
          monitor::enabledMonitors.end())
    ...
    ROS_INFO("Node %s successfully getMonitors from %s", node_name.c_str(),
             ci.ip.c_str());
    return true;
  } else {
    ROS_WARN(
        "Node %s is not able to getMonitors from %s due to access control!",
        node_name.c_str(), ci.ip.c_str());
...
*/
bool ServerManager::getRVStateCallback(XmlRpc::XmlRpcValue &params,
                                       ClientInfo &ci,
                                       XmlRpc::XmlRpcValue &result) {
  ...
  if (acctrl::isCommandAllowed(command, node_name, ci.ip)) {
    ...
    */
    ...
    ROS_INFO("Node %s successfully getRVState from %s", node_name.c_str(),
             ci.ip.c_str());
    return true;
  } else {
    ROS_WARN("Node %s is not able to getRVState from %s due to access control!",
             node_name.c_str(), ci.ip.c_str());
...
bool ServerManager::monitorControlCallback(XmlRpc::XmlRpcValue &params,
                                           ClientInfo &ci,
                                           XmlRpc::XmlRpcValue &result) {
  ...
  if (acctrl::isCommandAllowed(command, node_name, ci.ip)) {
    ...
    ROS_INFO("Node %s successfully monitorControl from %s", node_name.c_str(),
             ci.ip.c_str());
    return true;
  } else {
    ROS_WARN(
        "Node %s is not able to control monitor from %s due to access control!",
        node_name.c_str(), ci.ip.c_str());
...
void ServerManager::monitorControl(std::string monitor, bool enable) {
  ...
  /*
     ...
     rv::monitor::map_monitorTopics.find(monitor)->second;
     ...
     topics.begin();it!=topics.end();++it)
        {
           ...
           else //disable monitor
           {
              ...
              //cannot shutdown monitor, otherwise, existing connection between
     pub/sub would be broken
              //monitor_p->shutdown();
...
bool ServerManager::getSystemStateCallback(XmlRpc::XmlRpcValue &params,
                                           ClientInfo &ci,
                                           XmlRpc::XmlRpcValue &result) {
  // 0: node_name
  ...
  if (acctrl::isCommandAllowed(command, node_name, ci.ip)) {
    ...
    ROS_INFO("Node %s successfully getSystemState from %s", node_name.c_str(),
             ci.ip.c_str());
    return true;
  } else {
    ROS_WARN(
        "Node %s is not able to getSystemState from %s due to access control!",
        node_name.c_str(), ci.ip.c_str());
...
bool ServerManager::lookupServiceCallback(XmlRpc::XmlRpcValue &params,
                                          ClientInfo &ci,
                                          XmlRpc::XmlRpcValue &result) {
  // 0: node_name
  // 1: service name
  ...
  ROS_INFO("Node %s trying to lookup service %s from %s", node_name.c_str(),
  ...
  if (acctrl::isCommandAllowed(command, node_name, ci.ip)) {
    ...
    ROS_INFO("Node %s successfully lookup-ed service %s from %s",
             node_name.c_str(), service.c_str(), ci.ip.c_str());
    return true;
  } else {
    ROS_WARN("Node %s is not able to lookup service %s from %s due to access "
             "control!",
             node_name.c_str(), service.c_str(), ci.ip.c_str());
...
bool ServerManager::lookupNodeCallback(XmlRpc::XmlRpcValue &params,
                                       ClientInfo &ci,
                                       XmlRpc::XmlRpcValue &result) {
  // 0: node_name
  // 1: lookup node name
  ...
  ROS_INFO("Node %s trying to lookup node %s from %s", node_name.c_str(),
  ...
  if (acctrl::isCommandAllowed(command, node_name, ci.ip)) {
    ...
    string uri = result[2];
    ROS_INFO("Node %s successfully lookup-ed node %s with uri %s from %s",
             node_name.c_str(), lookup_node_name.c_str(), uri.c_str(),
             ci.ip.c_str());
    return true;
  } else {
    ROS_WARN(
        "Node %s is not able to lookup node %s from %s due to access control!",
        node_name.c_str(), lookup_node_name.c_str(), ci.ip.c_str());
...
bool ServerManager::registerServiceCallback(XmlRpc::XmlRpcValue &params,
                                            ClientInfo &ci,
                                            XmlRpc::XmlRpcValue &result) {
  string node_name = params[0];
  string service = params[1];
  ...
  ROS_INFO("Node %s trying to register service %s at address %s from %s",
           node_name.c_str(), service.c_str(), service_uri.c_str(),
           uri.c_str());

  XmlRpc::XmlRpcValue payload;
  master::execute("registerService", params, result, payload, true);

  ROS_INFO("Node %s successfully registered service %s from %s",
           node_name.c_str(), service.c_str(), ci.ip.c_str());
  return true;
  ...
  ROS_INFO("Node %s successfully registered service %s", node_name.c_str(),
  service.c_str());
  return true;
  }
  else
  {
  ROS_WARN("Node %s is not able to subscribe service %s due to access
  control!",node_name.c_str(), service.c_str());
  return false;
  }*/
}
...
void ServerManager::createInterceptors() {
  ...
  for (std::map<std::string, std::string>::iterator it =
           rv::monitor::topicsAndTypes.begin();
    ...
    ROS_INFO("Monitor %s successfully registered a subscriber to topic %s",
             monitorname.c_str(), topic.c_str());
  }
}

bool ServerManager::registerSubscriberCallback(XmlRpc::XmlRpcValue &params,
                                               ClientInfo &ci,
                                               XmlRpc::XmlRpcValue &result) {
  string node_name = params[0];
  ...
  ROS_WARN("Node %s trying to subscribe to topic %s from %s with datatype %s",
           node_name.c_str(), topic.c_str(), ci.ip.c_str(), datatype.c_str());
  ...
  if (acctrl::isSubscriberAllowed(topic, node_name, ci.ip)) // ip address
  {
    ...
    if (monitorMap.find(topic) != monitorMap.end()) {
      ROS_WARN("Node %s trying to subscribe to topic %s (MONITOR) ",
    ...
    } else if (rv::monitor::monitorTopics.find(topic) !=
               rv::monitor::monitorTopics.end()) {
      string monitorname = topic + MONITOR_POSTFIX;
      ROS_WARN("Node %s trying to subscribe to topic %s (NO-MONITOR) ",
               node_name.c_str(), topic.c_str());
      ...
      */
      ...
      ROS_INFO("Node %s received code %d with status %s", node_name.c_str(),
               code, status.c_str());
      ...
      ROS_INFO("Node %s successfully registered a subscriber to topic %s",
               node_name.c_str(), topic.c_str());
      return true;
    } else { // skip monitoring

      master::execute("registerSubscriber", params, result, payload, true);

      ROS_INFO("NO MONITORING - Node %s successfully registered a subscriber "
               "to topic %s",
               node_name.c_str(), topic.c_str());
      return true;
    }
    ...
    ROS_WARN(
        "Node %s is not able to subscribe to topic %s due to access control!",
        node_name.c_str(), topic.c_str());
    return false;
  }
}

bool ServerManager::unregisterServiceCallback(XmlRpc::XmlRpcValue &params,
                                              ClientInfo &ci,
                                              XmlRpc::XmlRpcValue &result) {
  string name = params[0];
  string service = params[1];
  ROS_INFO("Node %s unregister service %s from %s", name.c_str(),
           service.c_str(), ci.ip.c_str());
  XmlRpc::XmlRpcValue payload;
  master::execute("unregisterService", params, result, payload, true);
  return true;
}
bool ServerManager::unregisterSubscriberCallback(XmlRpc::XmlRpcValue &params,
                                                 ClientInfo &ci,
                                                 XmlRpc::XmlRpcValue &result) {
  ...
  string topic = params[1];
  string uri = params[2];
  ROS_INFO("Node %s trying to unregister as a subscriber to topic %s from %s",
           node_name.c_str(), topic.c_str(), ci.ip.c_str());
  ...
  if (acctrl::isSubscriberAllowed(topic, node_name, ci.ip)) {
    ...
    if (monitorMap.find(topic) != monitorMap.end()) {
      ...
      ROS_INFO("Node %s successfully unregistered as a subscriber to topic %s",
               node_name.c_str(), topic.c_str());
      return true;
    }
  } else {
    ROS_WARN("Node %s is not able to unregister subscriber to topic %s due to "
             "access control!",
             node_name.c_str(), topic.c_str());
 ...
 */
bool ServerManager::registerPublisherCallback(XmlRpc::XmlRpcValue &params,
                                              ClientInfo &ci,
                                              XmlRpc::XmlRpcValue &result) {
  ...
  ROS_INFO("Node %s trying to publish to topic %s from %s", node_name.c_str(),
           topic.c_str(), ci.ip.c_str());
  // ROS_INFO("Real ip address: %s  port: %d", ci.ip.c_str(), ci.port);
  ...
  if (acctrl::isPublisherAllowed(topic, node_name, ci.ip)) {
    ...
    if (monitor::monitorTopics.count(topic) == 0) {
      ROS_WARN("Node %s trying to publish to \"REGULAR\" topic %s",
               node_name.c_str(), topic.c_str());
    } else {
      ROS_WARN("Node %s trying to publish to \"MONITORED\" topic %s",
               node_name.c_str(), topic.c_str());
    }

    master::execute("registerPublisher", params, result, payload, true);

    ROS_INFO("Node %s successfully registered as a publisher to topic %s",
             node_name.c_str(), topic.c_str());

    return true;
  } else {
    ROS_WARN(
        "Node %s is not able to publish to topic %s due to access control!",
        node_name.c_str(), topic.c_str());
...
bool ServerManager::unregisterPublisherCallback(XmlRpc::XmlRpcValue &params,
                                                ClientInfo &ci,
                                                XmlRpc::XmlRpcValue &result) {
  ...
  ROS_INFO("Node %s trying to unregister as a publisher to topic %s from %s",
           node_name.c_str(), topic.c_str(), ci.ip.c_str());
  // ROS_INFO("Real ip address: %s  port: %d", ci.ip.c_str(), ci.port);
  ...
  if (acctrl::isPublisherAllowed(topic, node_name, ci.ip)) {
    ...
    ROS_INFO("Node %s successfully unregistered as a publisher to topic %s",
             node_name.c_str(), topic.c_str());
    return true;
  } else {
    ROS_WARN("Node %s is not able to unregister publish to topic %s due to "
             "access control!",
             node_name.c_str(), topic.c_str());
...
bool ServerManager::unsubscribeParamCallback(XmlRpc::XmlRpcValue &params,
                                             ClientInfo &ci,
                                             XmlRpc::XmlRpcValue &result) {
  ...
  ROS_INFO("Node %s trying to unsubscribeParam %s from %s", node_name.c_str(),
  ...
  if (acctrl::isCommandAllowed(command, node_name, ci.ip)) //??host or node_name
  {
    ...
    ROS_INFO("Node %s successfully unsubscribed to param %s from %s",
             node_name.c_str(), mapped_key.c_str(), ci.ip.c_str());
    return true;
  } else {
    ROS_WARN("Node %s is not able to unsubscribe to param %s from %s due to "
             "access control!",
             node_name.c_str(), mapped_key.c_str(), ci.ip.c_str());
...
bool ServerManager::subscribeParamCallback(XmlRpc::XmlRpcValue &params,
                                           ClientInfo &ci,
                                           XmlRpc::XmlRpcValue &result) {
  ...
  ROS_INFO("Node %s trying to subscribeParam %s from %s", node_name.c_str(),
           mapped_key.c_str(), ci.ip.c_str());
  ...
  if (acctrl::isCommandAllowed(command, node_name, ci.ip)) //??host or node_name
  {
    ...
    ROS_INFO("Node %s successfully subscribed to param %s from %s",
             node_name.c_str(), mapped_key.c_str(), ci.ip.c_str());
    return true;
  } else {
    ROS_WARN("Node %s is not able to subscribe to param %s from %s due to "
             "access control!",
             node_name.c_str(), mapped_key.c_str(), ci.ip.c_str());
...
bool ServerManager::hasParamCallback(XmlRpc::XmlRpcValue &params,
                                     ClientInfo &ci,
                                     XmlRpc::XmlRpcValue &result) {
  string name = params[0];
  string mapped_key = params[1];

  ROS_INFO("Node %s trying to hasParam %s from %s", name.c_str(),
           mapped_key.c_str(), ci.ip.c_str());
  ...
  if (acctrl::isCommandAllowed(command, name, ci.ip)) {
    ...
    ROS_WARN(
        "Node %s is not able to hasParam %s from %s due to access control!",
        name.c_str(), mapped_key.c_str(), ci.ip.c_str());
...
bool ServerManager::searchParamCallback(XmlRpc::XmlRpcValue &params,
                                        ClientInfo &ci,
                                        XmlRpc::XmlRpcValue &result) {
  ...
  ROS_INFO("Node %s trying to search parameter %s from %s", name.c_str(),
           mapped_key.c_str(), ci.ip.c_str());
  ...
  if (acctrl::isCommandAllowed(command, name, ci.ip)) {
    ...
    if (!hasParam) {
      result[0] = -1;
      result[1] =
          "Cannot find parameter [" + mapped_key + "] in an upwards search";
      result[2] = "";
    }

    // ROS_INFO("Node %s successfully searched parameter %s and return %s from
    // %s",name.c_str(), mapped_key.c_str(),(string(result[2])).c_str(),
    // ci.ip.c_str());
    return true;
  } else {

    ROS_WARN("Node %s is not able to search parameter %s from %s due to access "
             "control!",
             name.c_str(), mapped_key.c_str(), ci.ip.c_str());
...
bool ServerManager::setParamCallback(XmlRpc::XmlRpcValue &params,
                                     ClientInfo &ci,
                                     XmlRpc::XmlRpcValue &result) {
  ...
  ROS_INFO("Node %s trying to set parameter %s from %s", name.c_str(),
           mapped_key.c_str(), ci.ip.c_str());
  ...
  if (acctrl::isCommandAllowed(command, name, ci.ip)) {
    ...
    // ROS_INFO("Node %s succesfully set parameter %s to value %s from
    // %s",name.c_str(), mapped_key.c_str(), value.c_str(), ci.ip.c_str());
    return true;
  } else {

    ROS_WARN("Node %s is not able to set parameter %s from %s due to access "
             "control!",
             name.c_str(), mapped_key.c_str(), ci.ip.c_str());
...
bool ServerManager::getParamNamesCallback(XmlRpc::XmlRpcValue &params,
                                          ClientInfo &ci,
                                          XmlRpc::XmlRpcValue &result) {
  string name = params[0];

  ROS_INFO("Node %s trying to getParamNames from %s", name.c_str(),
           ci.ip.c_str());
  ...
  if (acctrl::isCommandAllowed(command, name, ci.ip)) {
    ...
    ROS_WARN(
        "Node %s is not able to getParamNames from %s due to access control!",
        name.c_str(), ci.ip.c_str());
...
bool ServerManager::getParamCallback(XmlRpc::XmlRpcValue &params,
                                     ClientInfo &ci,
                                     XmlRpc::XmlRpcValue &result) {
  string name = params[0];
  string mapped_key = params[1];

  ROS_INFO("Node %s trying to get parameter %s from %s", name.c_str(),
           mapped_key.c_str(), ci.ip.c_str());
  string command = "getParam"; //+mapped_key;
  if (acctrl::isCommandAllowed(command, name, ci.ip)) {
    ...
    ROS_WARN("Node %s is not able to get parameter %s from %s due to access "
             "control!",
             name.c_str(), mapped_key.c_str(), ci.ip.c_str());
...
bool ServerManager::deleteParamCallback(XmlRpc::XmlRpcValue &params,
                                        ClientInfo &ci,
                                        XmlRpc::XmlRpcValue &result) {
  string name = params[0];
  string mapped_key = params[1];

  ROS_INFO("Node %s trying to delete parameter %s from %s", name.c_str(),
           mapped_key.c_str(), ci.ip.c_str());
  ...
  if (acctrl::isCommandAllowed(command, name, ci.ip)) {
    ...
    ROS_WARN("Node %s is not able to delete parameter %s from %s due to access "
             "control!",
             name.c_str(), mapped_key.c_str(), ci.ip.c_str());
...
for testing only
*/
bool ServerManager::testxmlCallback(XmlRpc::XmlRpcValue &params, ClientInfo &ci,
                                    XmlRpc::XmlRpcValue &result) {
