...
public:
  XMLRPCCallWrapper(const std::string &function_name, const XMLRPCFunc &cb,
                    XmlRpcServer *s)
      : XmlRpcServerMethod2(function_name, s), name_(function_name), func_(cb) {
  }

  void execute(XmlRpc::XmlRpcValue &params, ClientInfo &ci,
...
int Monitor::removeSubscriber(std::string ip) {
  std::vector<std::string>::iterator it =
      std::find(sub_ips.begin(), sub_ips.end(), ip);
  if (it != sub_ips.end())
    sub_ips.erase(it);
...
SubscriptionPtr Monitor::getMonitorSubscriptionPtr() { return sub_ptr; }
ros::PublicationPtr Monitor::getMonitorPublicationPtr() { return pub_ptr; }
...
string Monitor::getMonitorName() { return m_name; }
int Monitor::getMonitorXmlRpcPort() { return m_port; }

int Monitor::getMonitorTcpPort() { return connection_manager_->getTCPPort(); }

Monitor::Monitor(XmlRpc::XmlRpcValue &params, string hostname,
                 string monitorname) {
  m_host = hostname;
  m_name = monitorname;
  ...
  // ops_pub.datatype, ops_pub.md5sum, ops_pub.message_definition,
  // ops_pub.queue_size, ops_pub.latch, ops_pub.has_header));

  // ros::SubscriberCallbacksPtr callbacks(new
  // ros::SubscriberCallbacks(ops_pub.connect_cb, ops_pub.disconnect_cb,
  ...
  // bind xmlrpc call
  bind("requestTopic",
       boost::bind(&Monitor::requestTopicCallback, this, _1, _2, _3));
  bind("publisherUpdate",
       boost::bind(&Monitor::pubUpdateCallback, this, _1, _2, _3));
  bind("shutdown", boost::bind(&Monitor::shutdownCallback, this, _1, _2, _3));
  bind("getBusInfo",
       boost::bind(&Monitor::getBusInfoCallback, this, _1, _2, _3));
  ...
  poll_manager->addPollThreadListener(
      boost::bind(&Monitor::processPublishQueues, this));
  ...
  SubscriptionPtr s(new Subscription(ops_sub.topic, ops_sub.md5sum,
                                     ops_sub.datatype, ops_sub.transport_hints,
                                     poll_manager));
  s->addCallback(ops_sub.helper, ops_sub.md5sum, ops_sub.callback_queue,
                 ops_sub.queue_size, ops_sub.tracked_object,
                 ops_sub.allow_concurrent_callbacks);
...
string Monitor::getMonitorXmlRpcUri() { return m_uri; }

bool Monitor::getBusInfoCallback(XmlRpc::XmlRpcValue &params, ClientInfo &ci,
                                 XmlRpc::XmlRpcValue &result) {
  string node_name = params[0];

  ROS_INFO("Node %s trying to getBusInfo from monitor %s", node_name.c_str(),
           m_name.c_str());
  ...
  if (acctrl::isCommandAllowed(command, node_name, ci.ip)) {
    ...
    ROS_INFO("Node %s successfully getBusInfo from monitor %s",
             node_name.c_str(), m_name.c_str());
    return true;
  } else {
    ROS_WARN("Node %s is not able to getBusInfo from monitor %s due to access "
             "control!",
             node_name.c_str(), m_name.c_str());
...
bool Monitor::shutdownCallback(XmlRpc::XmlRpcValue &params, ClientInfo &ci,
                               XmlRpc::XmlRpcValue &result) {
  string node_name = params[0];

  ROS_INFO("Node %s trying to shutdown monitor %s", node_name.c_str(),
           m_name.c_str());
  ...
  if (acctrl::isCommandAllowed(command, node_name, ci.ip)) {
    ...
    shutdown();
    ROS_INFO("Node %s successfully shutdown monitor %s", node_name.c_str(),
             m_name.c_str());
    result = rv::xmlrpc::responseInt(1, "monitor shutdown", 1);
    return true;
  } else {
    ROS_WARN(
        "Node %s is not able to shutdown monitor %s due to access control!",
        node_name.c_str(), m_name.c_str());
...
void Monitor::shutdown() {
  ...
  ROS_INFO("Monitor at TCP port %d was shutdown",
...
void Monitor::getPidCallback(XmlRpc::XmlRpcValue &params, ClientInfo &ci,
                             XmlRpc::XmlRpcValue &result) {
  result = xmlrpc::responseInt(1, "", (int)getpid());
}
void Monitor::requestTopicCallback(XmlRpc::XmlRpcValue &params, ClientInfo &ci,
  ...
  for (int proto_idx = 0; proto_idx < protos.size(); proto_idx++) {
    ...
    /*else if (proto_name == string("UDPROS"))
    {
      ...
      boost::shared_array<uint8_t> buffer = boost::shared_array<uint8_t>(new
    uint8_t[header_bytes.size()]);
    ...
    err.c_str());
      ...
      if(!pub_ptr)
      {
        ROSCPP_LOG_DEBUG("Unable to find advertised topic %s for UDPROS
    connection", topic.c_str());
        return false;
      }
      ...
      {
        ROSCPP_LOG_DEBUG("Error validating header from [%s:%d] for topic [%s]:
    %s", host.c_str(), port, topic.c_str(), error_msg.c_str());
        return false;
      }

      int max_datagram_size = proto[4];
      int conn_id = connection_manager_->getNewConnectionID();
      ros::TransportUDPPtr transport =
    connection_manager_->getUDPServerTransport()->createOutgoing(host, port,
    conn_id, max_datagram_size);
...
/*
void Monitor::paramUpdateCallback(XmlRpc::XmlRpcValue& params, ClientInfo &ci,
XmlRpc::XmlRpcValue &result)
{
  result[0] = 1;
  result[1] = std::string("");
...
void Monitor::pubUpdateCallback(XmlRpc::XmlRpcValue &params, ClientInfo &ci,
                                XmlRpc::XmlRpcValue &result) {
...
XmlRpcClient *Monitor::getXMLRPCClient(const std::string &host, const int port,
